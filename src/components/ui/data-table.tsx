...
interface DataTableProps<TData, TValue> {
  ...
  handleRowClickAccessor: string
}

export function DataTable<TData, TValue>({ columns, data, handleRowClick, handleRowClickAccessor}: DataTableProps<TData, TValue>) {
            ...
            table.getRowModel().rows.map((row) => (
              <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
                {row.getVisibleCells().map((cell) => (
                  ...
                  // @ts-ignore
                  onClick={() => handleRowClick(cell.row.original[handleRowClickAccessor])} className='cursor-pointer'
